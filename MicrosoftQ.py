...
logEmp.hist(bins=40)
plt.xlabel("Log of Number of Employees",fontsize=10)
plt.ylabel("Frequency",fontsize=10)

logEmp1= np.log10(df2.Num_Employees)
logEmp1.hist(bins=40)
plt.xlabel("Log of Number of Employees",fontsize=10)
plt.ylabel("Frequency",fontsize=10)
...
first_ten= df3.sort_values(by='difference', ascending=True)
...
(df1.groupby('industry').EMPTOTAL.sum()/totalEmp).plot('bar')
plt.ylabel("Total Employees",fontsize=10)

df1.groupby('industry').USSALES.sum().plot(kind= 'bar')
plt.ylabel("Total US Sales",fontsize=10)

df1['salesperlabor']= df1.USSALES/df1.EMPTOTAL
df1.groupby('industry').salesperlabor.mean().plot('bar')
...
df2[df2.NASDAQ==False].Sector.value_counts().plot('bar')
